...
//            }
//            Main::log_write("收到退出信号,退出主进程");
//            exit();
//        });
//        swoole_process::signal(SIGCHLD, function ($signo) {
//            $rs = swoole_process::wait();
//            Main::log_write("收到子线程退出信号");
//            Main::log_write($rs);
//        });
...
//        self::params_http($opt);
    ...
    static public function params_p($opt)
    {
        ...
        if (isset($opt["p"]) && $opt["p"]) {
            self::$pid_file = $opt["p"] . "/pid";
        }
        //记录pid文件位置
        if (isset($opt["pid"]) && $opt["pid"]) {
            self::$pid_file = $opt["pid"] . "/pid";
        }
        if (empty(self::$pid_file)) {
            self::$pid_file = ROOT_PATH . "/pid";
        }
    }
...
//        });
    ...
    public function http_run($worker)
    {
        ...
        Main::log_write(print_r($worker,TRUE));
        $binpath = $_SERVER["_"];
